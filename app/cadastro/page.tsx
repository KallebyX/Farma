...
/** Patient self-registration: choose pharmacy + name/phone → WhatsApp OTP → hub. */
export default function CadastroPage() {
  ...
  // Search pharmacies as the user types (when none selected yet).
  useEffect(() => {
    if (pharmacy) return;
  ...
  async function register() {
    ...
    if (!pharmacy) { setErr("Escolha sua farmácia"); return; }
    setBusy(true);
    try {
      const res = await fetch("/api/patient-auth/register", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, phone, pharmacyId: pharmacy.id }),
      });
      const j = await res.json().catch(() => ({}));
        ...
        {step === "form" ? (
          <div className="space-y-3">
            ...
            <p className="text-center text-[11px] text-slate-400">
              Ao continuar você concorda em receber mensagens da sua farmácia (LGPD).
            </p>
            <p className="text-center text-xs text-slate-500">
              Já tem conta? <Link href="/entrar" className="font-medium text-emerald-700 hover:underline">Entrar</Link>
            </p>
          </div>
        ) : (
          <div className="space-y-3">
