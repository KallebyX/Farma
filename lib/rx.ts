...
export async function createPrescriptionFromFile(args: { patientId: string; pharmacyId: string; file: File; notes?: string | null }) {
  if (!(await storageConfigured())) return { ok: false as const, status: 503, error: "Armazenamento não configurado (defina em Integrações no banco ou SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY)" };
  const { file } = args;
